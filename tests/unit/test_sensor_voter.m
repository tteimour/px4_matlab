...
adis_id = uint32(3) * 65536 + uint32(3) * 256 + uint32(2);   % ImuADIS16470
assert(sel.gyro_device_id == adis_id, ...
    sprintf('ADIS16470 (priority 90) selected, got device_id=%d', sel.gyro_device_id));

%% Case 2: forcing the primary's confidence to zero (saturated error
%  density) causes failover to next-best. Validator.put() refreshes
%  last_t each tick, so the test pumps the error counter via direct
%  field manipulation, then steps once to re-evaluate selection.
...
assert(sel2.gyro_device_id ~= adis_id, ...
    'After saturating error density on the ADIS, selection should switch');
